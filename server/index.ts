...
interface Room {
  users: Map<string, { name: string, isHost: boolean }>
  files: any[]
  createdAt: number
  roomName: string
  hostId: string
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', (data: { roomCode: string, roomName: string, userName: string }, callback: (success: boolean) => void) => {
    if (rooms.has(data.roomCode)) {
      callback(false)
      return
    }
    
    const userMap = new Map<string, { name: string, isHost: boolean }>()
  ...
  socket.on('join-room', (data: { roomCode: string, userName: string }, callback: (success: boolean, data?: { files: any[], roomName: string }) => void) => {
    if (!rooms.has(data.roomCode)) {
      callback(false)
      return
    }
